...
const DashboardNavbar = () => {
    ...
    const { logout } = useAccountStore.getState();
    const handleLogout = () => {
        logout()
        router.push('/login')
    }
            ...
            <nav className="bg-white border-b border-gray-200 px-4 py-2.5 dark:bg-gray-800 dark:border-gray-700 fixed left-0 right-0 top-0 z-50">
                <div className="flex flex-wrap justify-between items-center">
                    ...
                    <div className="flex items-center lg:order-2">
                        <Dropdown
                            label={<Avatar alt="User settings" img="/default-user-profile.png" rounded />}
                            arrowIcon={false}
                            inline
                        >
                            <DropdownHeader>
                            ...
                            <DropdownDivider />
                            <DropdownItem icon={LogOut} onClick={handleLogout}>Sign out</DropdownItem>
                        </Dropdown>
                    </div>
                </div>
            </nav>
